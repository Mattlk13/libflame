...
/* > \verbatim */
/* > */
/* > ZGETSLS solves overdetermined or underdetermined scomplex linear systems */
...
void zgetsls_(char *trans, aocl_int_t *m, aocl_int_t *n, aocl_int_t *nrhs, dcomplex *a,
              aocl_int_t *lda, dcomplex *b, aocl_int_t *ldb, dcomplex *work,
              aocl_int_t *lwork, aocl_int_t *info)
{
#if FLA_ENABLE_ILP64
    aocl_lapack_zgetsls(trans, m, n, nrhs, a, lda, b, ldb, work, lwork, info);
#else
    aocl_int64_t m_64 = *m;
    aocl_int64_t n_64 = *n;
    ...
    aocl_lapack_zgetsls(trans, &m_64, &n_64, &nrhs_64, a, &lda_64, b, &ldb_64, work, &lwork_64,
                        &info_64);
...
void aocl_lapack_zgetsls(char *trans, aocl_int64_t *m, aocl_int64_t *n, aocl_int64_t *nrhs,
                         dcomplex *a, aocl_int64_t *lda, dcomplex *b, aocl_int64_t *ldb,
                         dcomplex *work, aocl_int64_t *lwork, aocl_int64_t *info)
{
    AOCL_DTL_TRACE_LOG_INIT
    AOCL_DTL_SNPRINTF("zgetsls inputs: trans %c, m %" FLA_IS ", n %" FLA_IS ", nrhs %" FLA_IS
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        aocl_blas_xerbla("ZGETSLS", &i__1, (ftnlen)7);
    ...
    else if(anrm == 0.)
    {
        ...
        aocl_lapack_zlaset("F", &maxmn, nrhs, &c_b1, &c_b1, &b[b_offset], ldb);
        goto L50;
    }
    brow = *m;
    if(tran)
    ...
    bnrm = aocl_lapack_zlange("M", &brow, nrhs, &b[b_offset], ldb, dum);
    ibscl = 0;
    ...
    if(*m >= *n)
    {
        ...
        aocl_lapack_zgeqr(m, n, &a[a_offset], lda, &work[lw2 + 1], &lw1, &work[1], &lw2, info);
        if(!tran)
        {
            /* Least-Squares Problem min || A * X - B || */
            /* B(1:M,1:NRHS) := Q**T * B(1:M,1:NRHS) */
            aocl_lapack_zgemqr("L", "C", m, nrhs, n, &a[a_offset], lda, &work[lw2 + 1], &lw1,
                               &b[b_offset], ldb, &work[1], &lw2, info);
            /* B(1:N,1:NRHS) := inv(R) * B(1:N,1:NRHS) */
            aocl_lapack_ztrtrs("U", "N", "N", n, nrhs, &a[a_offset], lda, &b[b_offset], ldb, info);
            if(*info > 0)
            {
                AOCL_DTL_TRACE_LOG_EXIT
                return;
        ...
        else
        {
            ...
            aocl_lapack_ztrtrs("U", "C", "N", n, nrhs, &a[a_offset], lda, &b[b_offset], ldb, info);
            if(*info > 0)
            {
                AOCL_DTL_TRACE_LOG_EXIT
                return;
            ...
            aocl_lapack_zgemqr("L", "N", m, nrhs, n, &a[a_offset], lda, &work[lw2 + 1], &lw1,
                               &b[b_offset], ldb, &work[1], &lw2, info);
            scllen = *m;
        }
    }
    else
    {
        ...
        aocl_lapack_zgelq(m, n, &a[a_offset], lda, &work[lw2 + 1], &lw1, &work[1], &lw2, info);
        /* workspace at least M, optimally M*NB. */
        if(!tran)
        {
            /* underdetermined system of equations A * X = B */
            /* B(1:M,1:NRHS) := inv(L) * B(1:M,1:NRHS) */
            aocl_lapack_ztrtrs("L", "N", "N", m, nrhs, &a[a_offset], lda, &b[b_offset], ldb, info);
            if(*info > 0)
            {
                AOCL_DTL_TRACE_LOG_EXIT
                return;
            ...
            aocl_lapack_zgemlq("L", "C", n, nrhs, m, &a[a_offset], lda, &work[lw2 + 1], &lw1,
                               &b[b_offset], ldb, &work[1], &lw2, info);
        ...
        else
        {
            ...
            aocl_lapack_zgemlq("L", "N", n, nrhs, m, &a[a_offset], lda, &work[lw2 + 1], &lw1,
                               &b[b_offset], ldb, &work[1], &lw2, info);
            /* workspace at least NRHS, optimally NRHS*NB */
            /* B(1:M,1:NRHS) := inv(L**T) * B(1:M,1:NRHS) */
            aocl_lapack_ztrtrs("L", "C", "N", m, nrhs, &a[a_offset], lda, &b[b_offset], ldb, info);
            if(*info > 0)
            {
                AOCL_DTL_TRACE_LOG_EXIT
                return;
